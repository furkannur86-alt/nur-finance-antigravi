...
const ALL_VIEWS: ViewTab[] = [
  ...
  { id: "tatar-finans", label: "🛡️ Tatar Finans", group: "trading", adminOnly: true },
  { id: "nur-kids", label: "👨‍👩‍👧 Nur Kids", group: "nfs" },
  { id: "nur-education", label: "🎓 Education", group: "nfs" },
  { id: "compute-access", label: "⛏️ Free Access", group: "nfs" },
  ...
  { id: "nur-coin", label: "🪙 $NUR Coin", group: "trading" },
...
export default function TopBar() {
  ...
  // On sign-in, pull the persisted tier back from Supabase so a returning user's
  // access reflects their real profile row, not just fresh in-memory state.
  useEffect(() => {
    if (!user) return;
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="px-2.5 py-1 mr-2 rounded text-[10px] font-mono flex items-center gap-1.5 bg-black/40 hover:bg-black/60 border border-white/10 text-slate-300 transition-colors shrink-0"
        title="Hesap"
      >
        <span className={user ? "text-emerald-400" : "text-slate-400"}>👤</span>
        <span className="font-bold">{user ? user.email?.split("@")[0] : "Giriş Yap"}</span>
      </button>
