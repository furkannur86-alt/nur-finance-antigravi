...
// Deterministic pseudo-random (same seed -> same output), so useMemo bodies that use
// it stay pure — unlike Math.random(), which breaks memoization/purity guarantees.
...
export default function OMSEMSPanel() {
  ...
  // Reset price/stop-price defaults when the selected symbol changes, without an
  // effect — this is React's own recommended "adjust state during render" pattern
  // for resetting state in response to a prop/derived-value change.
  const [pricedSymbol, setPricedSymbol] = useState(selectedSymbol);
  if (selectedSymbol !== pricedSymbol) {
    setPricedSymbol(selectedSymbol);
    setPrice(currentAsset.price);
    setStopPrice(Number((currentAsset.price * 0.96).toFixed(2)));
  }

  // Generate dynamic L2 Depth of Market ladder around current price (deterministic
  // pseudo-random, seeded by price/level — keeps this useMemo a pure function of
  // currentAsset.price, its only dependency).
  const domData = useMemo(() => {
    ...
    for (let i = 5; i >= 1; i--) {
      ...
      const size = Math.floor(seededRandom(base + i) * 400 + 100);
      asks.push({ price: p, size, total: size * (6 - i) });
    }

    for (let i = 1; i <= 5; i++) {
      const p = Number((base - i * tick).toFixed(2));
      const size = Math.floor(seededRandom(base - i + 0.5) * 450 + 120);
