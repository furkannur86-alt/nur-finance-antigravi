    ...
    try {
      if (selectedTier === "NUR_FINANCE_R") {
        if (!isReutersEligible) {
          updateVerification({ tier: "NUR_FINANCE_R", reutersUsageMonths: reutersMonths, overallStatus: "REJECTED" });
          addNotification({
        ...
        // Hard requirement (history length) passed automatically; the attestation
        // document itself still needs human review before full access is granted.
        updateVerification({
          tier: "NUR_FINANCE_R",
        ...
        if (!isBloombergHistoryEligible) {
          updateVerification({ tier: "NUR_FINANCE_B", bloombergUsageMonths: bloombergMonths, overallStatus: "REJECTED" });
          addNotification({
        ...
        if (!inviteResult.valid) {
          ...
          addNotification({
            title: "Verification Denied: NUR Finance B",
            message: inviteResult.detail || "Prerequisite 2 failed: Valid VIP handpicked invitation code required.",
            severity: "CRITICAL",
            category: "COMPLIANCE",
          });
          return;
          ...
          <span
            className={`px-2 py-0.5 rounded font-bold ${
              verification.overallStatus === "VERIFIED"
                ...
                : verification.overallStatus === "UNDER_REVIEW"
          ...
          <form onSubmit={handleVerifySubmission} className="p-5 rounded-lg border bg-black/40 space-y-4" style={{ borderColor: "var(--ag-border)" }}>
            ...
            <div>
              <label className="text-[11px] font-semibold text-[var(--ag-muted)] uppercase block mb-1">
                Proof of Usage Attestation (Invoice / Statement PDF) — Required
              </label>
              <div className="flex items-center gap-3">
